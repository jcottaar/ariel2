...
import ariel_support as ars # used for some support functions

if ars.running_on_kaggle:
...
if ars.running_on_kaggle == 1:
    ...
    debugging_mode = 1
# 0: no checks, 1: limited checks, 2: heavy checks
# I'm not sure if debugging_mode 2 works at this point, haven't used it in a while...
...
class PriorMatrices(Superclass):
...
def solve_gp(model, obs, rng=None, n_samples=0, fill_noise_parameters=True):   
    ...
    if np.any(np.isnan(labels_with_offset)):
        raise ars.ArielException(5, 'NaNs in GP solve labels')
    ...
    if np.any(np.isnan(mean)):
        raise ars.ArielException(5, 'NaNs in GP solve output')
    ...
    if n_samples == 0:
        ...
        model_samples.set_parameters(samples)
        if debugging_mode >= 2 and model.is_linear():
